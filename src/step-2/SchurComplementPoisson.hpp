#ifndef H_SCHURCOMPLEMENTPOISSON__
#define H_SCHURCOMPLEMENTPOISSON__
...
private:
  const dealii::
  SmartPointer<const dealii::SparseMatrixEZ<heat::real> > constraintMatrix;
  
  const dealii::
  SmartPointer<const dealii::SparseMatrix<heat::real> > stiffnessMatrix;
  
  const dealii::
  SmartPointer<const dealii::IterativeInverse<dealii::Vector<heat::real> > > 
  massInverse;
